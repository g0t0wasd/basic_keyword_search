...
def normalize_string(pattern, string):
    """ Makes list of words from string,
...
class DataLoader:
    ...
    def get_id_name_norm(self, entry):
        ...
        category = entry[2]
        normalized_name = normalize_string(r"[., \-!?:()]+", entry[1])
        return book_id, original_name, normalized_name, category

    def normalize_data(self):
        """ Sets data as a dict with ID as a key and tuple
            (name, normalized name) as value """
        for entry in self.data:
            (book_id,
             original_name,
             normalized_name,
             category) = self.get_id_name_norm(entry)
            self.normalized_data[book_id] = (
                original_name, normalized_name, category
                )
...
class Searcher:
    ...
    def search(self, query):
        ...
        normalized_query = normalize_string(r"[., \-!?:()]+", query)
...
class InvertedSolution(Searcher):
    ...
    def search(self, query):
        ...
        for book_id in book_ids:
            ...
            matched, missed = ranker.match_not_match(normalized_query,
                                                     normalized_name)
